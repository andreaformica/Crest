...
public class RuninfoApi  {
   ...
   protected JAXRSContext context;
    ...
    public Response listRunInfo(@ApiParam(value = "since: the starting time or run-lumi", defaultValue = "none") @DefaultValue("none") @QueryParam("since")  String since,@ApiParam(value = "until: the ending time or run-lumi", defaultValue = "none") @DefaultValue("none") @QueryParam("until")  String until,@ApiParam(value = "the format to digest previous arguments [iso], [number], [run-lumi]. Time(iso) = yyyymmddhhmiss,  Time(number) = milliseconds or Run(number) = runnumber Run(run-lumi) = runnumber-lumisection ", defaultValue = "number") @DefaultValue("number") @QueryParam("format")  String format,@ApiParam(value = "the mode for the request : [daterange] or [runrange] ", allowableValues="daterange, runrange", defaultValue = "runrange") @DefaultValue("runrange") @Pattern(regexp="daterange|runrange") @QueryParam("mode")  String mode,@ApiParam(value = "page: the page number {0}", defaultValue = "0") @DefaultValue("0") @QueryParam("page")  Integer page,@ApiParam(value = "size: the page size {1000}", defaultValue = "1000") @DefaultValue("1000") @QueryParam("size")  Integer size,@ApiParam(value = "sort: the sort pattern {id.runNumber:ASC}", defaultValue = "id.runNumber:ASC") @DefaultValue("id.runNumber:ASC") @QueryParam("sort")  String sort,@Context SecurityContext securityContext,@Context UriInfo info)
    throws NotFoundException {
        context.setHttpHeaders(headers);
        context.setRequest(request);
        return delegate.listRunInfo(since, until, format, mode, page, size, sort, securityContext, info);
    }
    @PUT
